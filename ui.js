...
// --- NEWLY ADDED YOUTUBE SEARCH FUNCTIONS ---
async function searchYouTubeVideos(language, keywords) {
    try {
        updateVideoStatus('searchingForVideos');

        // Analyze content to determine the best search approach
        const enhancedKeywords = await analyzeContentForSearch(language, keywords);
        const searchQuery = `${language} ${enhancedKeywords} language learning tutorial`;
        const youtubeSearchUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(searchQuery)}`;

        return youtubeSearchUrl;
    } catch (error) {
        console.error('YouTube search failed:', error);
        updateVideoStatus('videoNotAvailable');
        return null;
    }
}

async function analyzeContentForSearch(language, originalKeywords) {
    try {
        const analysisPrompt = `
Analyze the following keywords for a ${language} language learning context: "${originalKeywords}"

Determine if the content relates to cultural, grammatical, historical, or other educational aspects. 
Based on your analysis, enhance the search keywords using ONE of these patterns:

- For cultural content: "${language} culture ${originalKeywords}"
- For grammar content: "${language} grammar ${originalKeywords}" 
- For historical content: "${language} history ${originalKeywords}"
- For general cultural context: "${language} culture ${originalKeywords}"
- For pronunciation: "${language} pronunciation ${originalKeywords}"
- For vocabulary: "${language} vocabulary ${originalKeywords}"

Response format: Just return the enhanced keywords, nothing else.
`;

        // Use the existing API to analyze content
        if (window.api && window.api.callGeminiAPI) {
            const response = await window.api.callGeminiAPI(analysisPrompt);
            const enhancedKeywords = response.candidates[0].content.parts[0].text.trim();

            // Fallback to original if analysis fails
            return enhancedKeywords || originalKeywords;
        }

        return originalKeywords;
    } catch (error) {
        console.error('Content analysis failed, using original keywords:', error);
        return originalKeywords;
    }
}