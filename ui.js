...
async function searchAndLoadYouTubeVideo(title) {
    ...
    try {
        ...
        // Generate intelligent search term using Gemini
        const searchQuery = await createIntelligentSearchTerm(title, content.body);
        console.log('Generated search query:', decodeURIComponent(searchQuery));
...
async function createIntelligentSearchTerm(explanationTitle, explanationBody) {
    try {
        // Get current language from the language select element
        const targetLanguage = domElements.languageSelect?.value || 'English';
        const nativeLang = getNativeLang() || 'en';
...
Given:
- Grammar/Language Topic: "${explanationTitle}"
- Explanation Content: "${explanationBody}"
- Target Language: "${targetLanguage}"
- User's Native Language: "${nativeLang}"

Instructions:
1. Analyze the explanation content for cultural references, historical events, or other context-specific information.
2. Based on your analysis, dynamically adjust the keywords to focus on cultural, grammatical, historical, or general explanations.
3. Construct an optimized YouTube search query that will find the best educational videos.

Consider:
- Common terms used by language educators on YouTube
- Alternative phrasings and synonyms
- Popular educational channels' naming conventions
